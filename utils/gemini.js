  ...
  async generate(contents, opts = {}) {
    ...
    for (let keyAttempt = 0; keyAttempt < n; keyAttempt++) {
      ...
      for (let retry = 0; retry < 3; retry++) {
        try {
          // Google's docs specify the key must go in the `x-goog-api-key`
          // header (works for both legacy AIza... standard keys and the
          // newer AQ... auth keys) — passing it as ?key= or as a Bearer
          // token both get rejected for auth keys.
